  ...
  useEffect(() => {
    const fetchWorkshops = async () => {
      try {
        ...
        const response = await axios.get(`https://rootsnwings-api-944856745086.europe-west2.run.app/classes/?mentorId=${user.user.uid}&type=workshop`);
        ...
        for (const workshop of workshops) {
          try {
            ...
            const bookingsResponse = await axios.get(`https://rootsnwings-api-944856745086.europe-west2.run.app/bookings/?classId=${workshop.classId}`);
            ...
            // Calculate earnings
            const workshopEarnings = confirmedBookings.reduce((sum, booking) => 
              sum + parseFloat(booking.pricing?.subtotal || workshop.pricing?.subtotal || 0), 0
            );
            ...
            const now = new Date();
            const workshopDate = workshop.schedule?.startDate ? new Date(workshop.schedule.startDate) : now;
            ...
            if (workshop.status === 'pending') {
              ...
              if (confirmedBookings.length >= (workshop.capacity?.minStudents || 1)) {
            ...
            console.error(`Error fetching bookings for workshop ${workshop.classId}:`, error);
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            {loading ? (
              ...
              <div className="col-span-2 text-center py-12">
                ...
                <h3 className="text-xl font-semibold text-gray-900 mb-2">No Active Workshops</h3>
                <p className="text-gray-600 mb-6">Create your first workshop to start hosting special events</p>
                <button 
              ...
              <div key={workshop.classId} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                <div className={`aspect-video bg-gradient-to-br ${getWorkshopGradient(workshop.subject)} relative`}>
                  <div className="absolute top-4 left-4">
                    <span className="bg-green-100 text-green-800 text-xs px-2 py-1 rounded-full font-medium">{workshop.status}</span>
                  </div>
                  <div className="absolute top-4 right-4">
                    <span className={`${tagColors[workshop.format === "online" ? "green" : workshop.format === "in-person" ? "blue" : "purple"]} text-xs px-2 py-1 rounded-full font-medium`}>{workshop.format === 'in-person' ? 'In-person' : workshop.format === 'online' ? 'Online' : 'Hybrid'}</span>
                  </div>
                  <div className="absolute inset-0 flex items-center justify-center">
                    <i className={`${getWorkshopIcon(workshop.subject)} text-4xl`}></i>
                  </div>
                </div>
                <div className="p-6">
                  ...
                  <div className="space-y-3 mb-4">
                    ...
                    <div className="flex items-center text-sm text-gray-600">
                      <i className={`${workshop.format === "online" ? "fas fa-globe" : "fas fa-map-marker-alt"} mr-3 text-gray-400`}></i>
                      <span>{workshop.format === 'online' ? 'Online via Zoom' : workshop.location || 'In-person'}</span>
                    </div>
                    <div className="flex items-center text-sm text-gray-600">
                      <i className="fas fa-users mr-3 text-gray-400"></i>
                      <span>{workshop.attendeeCount || 0}/{workshop.capacity?.maxStudents || 0} participants</span>
                    </div>
                    <div className="flex items-center text-sm text-gray-600">
                      <i className="fas fa-tag mr-3 text-gray-400"></i>
                      <span>
                        {workshop.pricing?.basePrice === 0 ? 'Free Workshop' : `£${workshop.pricing?.basePrice || 0} per person`}
                      </span>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2 mb-4">
                    <span className="bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded-full">{workshop.subject}</span>
                    <span className={`${tagColors[workshop.format === "online" ? "green" : "blue"]} text-xs px-2 py-1 rounded-full`}>
                      {workshop.format === 'in-person' ? 'In-person' : 'Online'}
                    </span>
                    {workshop.pricing?.basePrice === 0 && (
                      <span className="bg-yellow-100 text-yellow-800 text-xs px-2 py-1 rounded-full">Free</span>
                    )}
                  </div>
                  <div className="flex space-x-3">
                    <button 
                      onClick={() => window.location.href = `/mentor/classes/${workshop.classId}`}
          ...
          <div className="space-y-6">
            {loading ? (
              ...
              workshopData.upcoming.map(workshop => (
                <div key={workshop.classId} className="bg-white rounded-xl border border-yellow-300 p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-2">
                        <h3 className="text-xl font-bold text-gray-900">{workshop.title}</h3>
                        <span className="bg-yellow-100 text-yellow-800 text-xs px-2 py-1 rounded-full font-medium">Needs Students</span>
                      </div>
                      ...
                      <div className="grid md:grid-cols-2 gap-4 text-sm text-gray-600 mb-4">
                        ...
                        <div>
                          ...
                          {workshop.attendeeCount || 0}/{workshop.capacity?.minStudents || 0} needed
                        </div>
                      </div>
                      <div className="flex space-x-3">
                        <button className="bg-yellow-500 text-white px-4 py-2 rounded-lg hover:bg-yellow-600 transition-colors text-sm">
                          <i className="fas fa-bullhorn mr-2"></i>Promote Workshop
                        </button>
          ...
          <div className="space-y-6">
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-xl font-semibold text-gray-900 mb-2">No Past Workshops</h3>
                <p className="text-gray-600">Your completed workshops will appear here.</p>
              </div>
            ) : (
              workshopData.past.map(workshop => (
                <div key={workshop.classId} className="bg-white rounded-xl border border-green-300 p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-2">
                        <h3 className="text-xl font-bold text-gray-900">{workshop.title}</h3>
          ...
          <div className="space-y-6">
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-xl font-semibold text-gray-900 mb-2">No Draft Workshops</h3>
              ...
              workshopData.drafts.map(workshop => (
                <div key={workshop.classId} className="bg-white rounded-xl border border-gray-300 p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-2">
                        <h3 className="text-xl font-bold text-gray-900">{workshop.title}</h3>
                      ...
                      <div className="flex space-x-3">
                        <button 
                          onClick={() => window.location.href = `/mentor/hostaclass/edit/${workshop.classId}`}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            {loading ? (
              ...
              [{
                ...
                change: workshopStats.activeCount > 0 ? `+${workshopStats.activeCount}` : '0',
                ...
                change: workshopStats.totalEarnings > 0 ? `+£${workshopStats.totalEarnings.toFixed(0)}` : '£0',
                changeColor: "text-green-500"
              }].map((stat, index) => (
