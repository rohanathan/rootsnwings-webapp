...
export default function Students() {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        ...
        const classesResponse = await axios.get(`https://rootsnwings-api-944856745086.europe-west2.run.app/classes/?mentorId=${user.user.uid}`);
        ...
        for (const classObj of mentorClasses) {
          try {
            ...
            const confirmedBookings = bookings.filter(booking => 
              booking.bookingStatus === 'approved' || booking.bookingStatus === 'confirmed' || booking.paymentStatus === 'paid'
            );

            console.log(`Confirmed bookings after filter: ${confirmedBookings.length}`);
            ...
            for (const booking of confirmedBookings) {
              ...
              try {
                ...
                if (!studentsMap.has(booking.studentId)) {
                  ...
                  try {
                    const studentResponse = await axios.get(`https://rootsnwings-api-944856745086.europe-west2.run.app/users/${booking.studentId}`);
                    studentProfile = studentResponse.data.user;
                    if (studentProfile) {
                      studentName = studentProfile.displayName || `${studentProfile.firstName} ${studentProfile.lastName}` || studentName;
                    }
                  } catch (profileError) {
                    console.log(`Could not fetch profile for student ${booking.studentId}, using booking data`);
                  }
                  ...
                  if (classObj.type === 'workshop') {
                    classType = 'workshop';
                    totalWorkshop++;
                  } else if (classObj.type === 'one-on-one' || classObj.capacity?.maxStudents === 1) {
  ...
  const getRandomBgColor = () => {
    const colors = ['bg-primary', 'bg-purple-500', 'bg-green-500', 'bg-primary-dark', 'bg-yellow-500', 'bg-red-500', 'bg-indigo-500'];
  ...
  const filteredStudents = students.filter(student => {
    const matchesFilter = activeFilter === 'all' || student.type === activeFilter;
            ...
            <div className="flex items-center space-x-4 mb-6">
              <div className="flex space-x-2">
                ...
                <button 
                  ...
                  1-on-1 ({loading ? '...' : students.filter(s => s.type === 'one-on-one').length})
                </button>
                <button 
                  ...
                  data-filter="group">
                  Group Classes ({loading ? '...' : students.filter(s => s.type === 'group').length})
                </button>
                <button 
                  ...
                  Workshop Attendees ({loading ? '...' : students.filter(s => s.type === 'workshop').length})
                </button>
              </div>
                          ...
                          <div className="flex-1">
                            <div className="flex items-center space-x-3 mb-1">
                              ...
                              <span className={`text-xs px-2 py-1 rounded-full font-medium ${
                                student.level === 'Beginner' ? 'bg-blue-100 text-blue-800' : 
                                student.level === 'Advanced' ? 'bg-red-100 text-red-800' : 
                                'bg-purple-100 text-purple-800'
                              }`}>
                                {student.level}
                              </span>
                              <span className={`text-xs px-2 py-1 rounded-full font-medium ${
                                student.type === 'one-on-one' ? 'bg-purple-100 text-purple-800' :
                                student.type === 'workshop' ? 'bg-orange-100 text-orange-800' :
                                'bg-indigo-100 text-indigo-800'
                              }`}>
                                {student.type === 'one-on-one' ? '1-on-1' : student.type === 'workshop' ? 'Workshop' : 'Group'}
                              </span>
                            </div>
                          ...
                          <div className="flex space-x-2">
                            {student.actionBtns.map((btn, btnIndex) => (
                              <button 
                                ...
                                className={`px-4 py-2 rounded-lg text-sm transition-colors ${btn.primary ? 'bg-primary text-white hover:bg-primary-dark' : 'border border-gray-300 text-gray-700 hover:bg-gray-50'}`}
                              >
