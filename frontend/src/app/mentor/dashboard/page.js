...
const quickStats = [
  ...
  { icon: 'fas fa-user-graduate', title: 'Students Enrolled', value: '24', change: '+3', iconBg: 'bg-green-100', iconColor: 'text-green-600', changeColor: 'text-green-500' },
  { icon: 'fas fa-calendar-check', title: 'Upcoming Sessions', value: '8', change: 'This week', iconBg: 'bg-purple-100', iconColor: 'text-purple-600', changeColor: 'text-purple-500' },
  { icon: 'fas fa-pound-sign', title: 'Earnings This Month', value: '£1,840', change: '+£240', iconBg: 'bg-yellow-100', iconColor: 'text-yellow-600', changeColor: 'text-green-500' },
];

const upcomingSessions = [
  { name: 'Ananya Mehta', subject: 'Kathak Basics • 1-on-1', time: 'Today • 3:00 PM - 4:00 PM', initials: 'AM', color: 'bg-primary', action: 'Join' },
  { name: 'Raj Kumar', subject: 'Classical Dance Workshop', time: 'Tomorrow • 10:00 AM - 12:00 PM', initials: 'RK', color: 'bg-green-500', action: 'Reschedule' },
  { name: 'Shreya Patel', subject: 'Advanced Techniques • 1-on-1', time: 'Wednesday • 6:00 PM - 7:00 PM', initials: 'SP', color: 'bg-purple-500', action: 'Prepare' },
];

const activeClasses = [
  { title: 'Weekend Kathak Batch (8 weeks)', status: 'Active', statusColor: 'bg-green-100 text-green-800', students: '6/8 students', schedule: 'Sat & Sun • 10 AM', progress: 'Week 3 of 8', actions: ['Manage Class', 'View Students'] },
  { title: 'Beginner Classical Dance', status: 'Waiting', statusColor: 'bg-yellow-100 text-yellow-800', students: '2/6 students', schedule: 'Weekdays • 5 PM', progress: 'Need 4 more', actions: ['Promote Class', 'Edit Details'] },
];

const pendingTasks = [
  { text: 'Complete profile verification', subtext: 'Upload DBS certificate', color: 'bg-red-50', dotColor: 'bg-red-500', action: 'Complete' },
  { text: 'Respond to messages', subtext: '3 unread from students', color: 'bg-yellow-50', dotColor: 'bg-yellow-500', action: 'View' },
  { text: 'Add profile picture', subtext: 'Increase student trust', color: 'bg-blue-50', dotColor: 'bg-blue-500', action: 'Upload' },
];
            ...
            <div className="bg-gradient-to-r from-primary to-primary-dark rounded-2xl p-8 text-white mb-8">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-blue-100 text-lg mb-4">Let's inspire students today.</p>
              ...
              <div className="lg:col-span-2 space-y-8">
                ...
                <div className="bg-white rounded-xl p-6 border border-gray-200">
                  <div className="flex items-center justify-between mb-6">
                    <h2 className="text-xl font-bold text-gray-900">Your Next Sessions</h2>
                  ...
                  <div className="space-y-4">
                    {upcomingSessions.map((session, index) => (
                      <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                        <div className="flex items-center space-x-4">
                          <div className={`w-12 h-12 ${session.color} rounded-full flex items-center justify-center`}>
                            <span className="text-white font-semibold text-sm">{session.initials}</span>
                          </div>
                          <div>
                ...
                <div className="bg-white rounded-xl p-6 border border-gray-200">
                  <div className="flex items-center justify-between mb-6">
                    <h2 className="text-xl font-bold text-gray-900">Active Classes & Batches</h2>
                    <a href="#" className="text-primary hover:underline text-sm font-medium">Manage all</a>
                  </div>
                    ...
                    {activeClasses.map((aclass, index) => (
                      <div key={index} className="border border-gray-200 rounded-lg p-4">
                        <div className="flex items-center justify-between mb-3">
                          <h3 className="font-semibold text-gray-900">{aclass.title}</h3>
                          <span className={`${aclass.statusColor} text-xs px-2 py-1 rounded-full font-medium`}>{aclass.status}</span>
                        </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl p-6 border border-gray-200">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="bg-red-100 text-red-800 text-xs px-2 py-1 rounded-full font-medium">{pendingTasks.length} new</span>
                  </div>
                  
                  <div className="space-y-3">
                    {pendingTasks.map((task, index) => (
                      <div key={index} className={`flex items-center space-x-3 p-3 ${task.color} rounded-lg`}>
                        <div className={`w-2 h-2 ${task.dotColor} rounded-full`}></div>
                        <div className="flex-1">
                          <p className="text-sm font-medium text-gray-900">{task.text}</p>
                          <p className="text-xs text-gray-500">{task.subtext}</p>
                        </div>
                        <button className="text-xs text-primary hover:underline">{task.action}</button>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-br from-green-50 to-blue-50 rounded-xl p-6 border border-green-200">
                  <div className="text-center">
                    ...
                    <h3 className="text-lg font-bold text-gray-900 mb-2">Want more students?</h3>
                    <p className="text-gray-600 text-sm mb-4">Boost your profile visibility and attract more learners!</p>
                ...
                <div className="bg-white rounded-xl p-6 border border-gray-200">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-gray-600 text-sm mb-4">Based on 47 reviews</p>
                      ...
                      {ratingData.map((rating, index) => (
                        <div key={index} className="flex items-center">
                          ...
                          <div className="flex-1 bg-gray-200 rounded-full h-2 mx-2">
                            <div className="bg-yellow-400 h-2 rounded-full" style={{ width: rating.width }}></div>
                          </div>
