...
export default function BookingConfirmation() {
  ...
  useEffect(() => {
    ...
    const loadClassData = async () => {
      ...
      try {
        ...
        const storedClass = JSON.parse(
          localStorage.getItem("selectedMentorClass") || "{}"
        );
        ...
        if (storedClass.classId && storedMentor.uid) {
          ...
          const pricing = calculateTotalPayable(
            storedClass,
            storedMentor,
            true
          ); // Assume first session for now
        ...
        const fallbackClass = JSON.parse(
          localStorage.getItem("selectedMentorClass") || "{}"
        );
        const fallbackMentor = JSON.parse(
  ...
  const applyPromoCode = () => {
    ...
    if (validCodes[code]) {
      ...
      const originalPrice =
        selectedMentorClass.pricing?.subtotal || bookingTotal;
      ...
      alert(
        `Promo code applied! ${discountPercentage}% discount saved you £${discount.toFixed(
          2
        )}`
      );
    } else {
  ...
  const handleAddToCalendar = () => {
    ...
    const title =
      "8-Week Weekend Batch - Intermediate Kathak with Priya Sharma";
    ...
    const calendarUrl = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(
      ...
      location
    )}&recur=RRULE:FREQ=WEEKLY;BYDAY=SA,SU;COUNT=16`;
                ...
                <div className="bg-white rounded-2xl p-6 shadow-lg sticky top-40">
                  <div className="text-center mb-6">
                    <div className="relative mx-auto mb-4">
                      <div className="w-24 h-24 bg-gradient-to-r from-primary to-primary-dark rounded-full flex items-center justify-center text-white text-3xl font-bold mx-auto">
                        P
                      </div>
                      <div className="absolute -bottom-2 -right-2 bg-green-500 text-white w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold">
                        ✓
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-center gap-2 mb-4">
                      ...
                      <span className="text-sm text-gray-600">
                        {mentor?.stats?.avgRating} (
                        {mentor?.stats?.totalReviews} reviews)
                      </span>
                    </div>
              ...
              <div className="lg:col-span-2">
                  ...
                  <div className="space-y-6 mb-8">
                        ...
                        <div>
                          ...
                          <div className="text-primary-dark font-bold">
                            {selectedMentorClass.type === "one-on-one"
                              ? "One-on-One Sessions"
                              : selectedMentorClass.type === "workshop"
                              ? "Workshop"
                              : "Group Class"}
                          </div>
                        </div>
                    ...
                    <div className="flex items-start">
                      ...
                      <div>
                        ...
                        <div className="text-primary-dark font-bold text-lg">
                          {selectedMentorClass.title || "Loading..."}
                        </div>
                      </div>
                    </div>
                    <div className="flex items-start">
                      ...
                      <div>
                        ...
                        <div className="text-primary-dark font-bold">
                          {(() => {
                            const startDate =
                              selectedMentorClass.schedule?.startDate;
                            const endDate =
                              selectedMentorClass.schedule?.endDate;
                        ...
                        {selectedMentorClass.type === "one-on-one" && (
                          <div className="text-sm text-gray-500">
                            {selectedMentorClass.pricing?.totalSessions || 1}{" "}
                            session
                            {(selectedMentorClass.pricing?.totalSessions || 1) >
                            1
                              ? "s"
                    ...
                    <div className="flex items-start">
                      ...
                      <div>
                        ...
                        <div className="text-primary-dark font-bold">
                          {selectedMentorClass.format === "online"
                            ? "Online"
                            : selectedMentorClass.format === "in-person"
                            ? "In-Person"
                            : selectedMentorClass.format === "hybrid"
                            ? "Hybrid"
                            : "Online"}
                          {selectedMentorClass.format === "in-person" &&
                            " (Birmingham)"}
                        ...
                        <div className="text-sm text-gray-500">
                          {selectedMentorClass.format === "online"
                            ...
                            : selectedMentorClass.format === "in-person"
                            ? "Community Centre, Birmingham B12 8JA"
                            : selectedMentorClass.format === "hybrid"
                    ...
                    <div className="space-y-3 mb-6">
                      <div className="flex justify-between">
                        <span className="text-gray-600">
                          {pricingBreakdown?.totalSessions ||
                            selectedMentorClass.pricing?.totalSessions ||
                            1}{" "}
                          Session
                          {(pricingBreakdown?.totalSessions ||
                            selectedMentorClass.pricing?.totalSessions ||
                        ...
                        <span className="font-semibold">
                          {formatPrice(
                            pricingBreakdown?.subtotal ||
                              (selectedMentorClass.pricing?.perSessionRate ||
                                0) *
                                (selectedMentorClass.pricing?.totalSessions ||
                                  1),
                            pricingBreakdown?.currency ||
                              selectedMentorClass.pricing?.currency ||
                              "GBP"
                          )}
                        </span>
                      </div>
                      ...
                      <div className="text-sm text-gray-500">
                        ...
                        per session
                        {bookingTotal === 0 &&
                        pricingBreakdown?.discountType === "first_session_free"
                          ? " (First session free!)"
                          : " (excellent value!)"}
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-50 p-4 rounded-xl mb-6">
                      ...
                      <div className="flex gap-3">
                        ...
                        <button
                          ...
                          disabled={isPromoApplied}
                          className={`bg-gray-200 text-gray-700 px-6 py-3 rounded-lg font-semibold transition-colors ${
                            !isPromoApplied
                              ? "hover:bg-gray-300"
                    ...
                    <div className="mb-8">
                      <label className="flex items-start gap-3 cursor-pointer">
                        ...
                        <div className="text-sm text-gray-700 leading-relaxed">
                          ...
                          . I understand that group classes have a 48-hour
                          cancellation policy.
                        </div>
                      </label>
                    </div>
                ...
                <div className="space-y-4 mb-8">
                  ...
                  >
                    {isProcessing
                      ? "Processing Payment..."
                      : `🔒 Pay & Confirm Booking - ${formatPrice(
                          bookingTotal,
                          pricingBreakdown?.currency ||
                            selectedMentorClass.pricing?.currency ||
                            "GBP"
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full text-center">
            ...
            <p className="text-gray-600 mb-6">
              Great! You're all set for the 8-Week Weekend Batch with mentor.
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                className="w-full border-2 border-gray-300 text-gray-700 py-3 rounded-full font-semibold transition-colors"
              >
                View My Bookings
              </button>
            </div>
        ...
        <div className="fixed top-20 right-5 z-50 max-w-sm w-full bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg shadow-lg">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-sm">
                This batch is now full. Please try another session or contact
                the mentor.
              </p>
            </div>
          </div>
